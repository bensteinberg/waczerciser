 ...
 */
export async function extractArchive(inputFile: string, outputDir: string): Promise<void> {
    if (inputFile.toLowerCase().endsWith('.wacz')) {
        return extractWACZ(inputFile, outputDir);
    } else if (inputFile.toLowerCase().endsWith('.warc') || inputFile.toLowerCase().endsWith('.warc.gz')) {
        return extractWARC(inputFile, outputDir);
    } else {
        throw new Error(`Unknown file type. Supported formats: .wacz, .warc, .warc.gz`);
    }
}
 ...
 * @throws Error if extraction fails
 */
export async function extractWACZ(waczFile: string, outputDir: string): Promise<void> {
    // Validate inputs
    ...
    try {
      for await (const entry of zip) {
        ...
        if (entry.filename.endsWith('/')) {
          ...
          await pipeline(await entry.openReadStream(), fs.createWriteStream(entryPath));
        }
      }
    } finally {
      await zip.close();
 ...
 * @throws Error if extraction fails
 */
export async function extractWARC(warcFile: string, outputDir: string): Promise<void> {
    // Import required modules
    ...
    const fullWarcPath = path.join(outputDir, path.basename(warcFile).replace(/\.gz$/, ''));
    ...
    for await (const record of parser) {
        ...
        fullWarcStream.write(await WARCSerializer.serialize(record, { gzip: false }));
    }
