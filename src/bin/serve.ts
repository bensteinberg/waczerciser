...
export async function serveCommand(archiveFile: string, options: ServeOptions) {
	const { replayFile, watcher, tempPrefix } = await setupArchiveFile(
		archiveFile,
		options,
	);
...
async function setupArchiveFile(archiveFile: string, options: ServeOptions) {
	...
	if (isDirectory) {
		const { suffix, asFiles } = determineArchiveFormat(
			archiveFile,
			options.format,
		);

		// Create unique temp file path
		tempPrefix = await fs.promises.mkdtemp(
			path.join(os.tmpdir(), "wacz-serve-"),
		);
		replayFile =
		...
		console.log(
			chalk.blue(
				`Building initial archive from ${asFiles ? "files" : suffix === ".wacz" ? "WACZ" : "WARC"} ...`,
			),
		);
...
function determineArchiveFormat(archiveDir: string, format?: string) {
	let suffix = ".warc.gz";
	let asFiles = false;
	...
	if (format) {
		...
		if (fs.existsSync(path.join(archiveDir, "datapackage.json"))) {
			...
			!fs.existsSync(path.join(archiveDir, "http:")) &&
