...
export function createCLI() {
	...
	program
		...
		.option(
			"-w, --watch",
			"Watch input directory and rebuild archive on changes",
		)
		.option(
			"--as-files",
			"Treat all files as file:// URLs instead of http(s):// URLs",
		)
	...
	program
		...
		.argument(
			"<archiveFile>",
			"Path to the WACZ/WARC file or directory to serve",
		)
		.option("-p, --port <number>", "Port to run the server on", "8080")
		.option("-u, --url <url>", "URL to start at", "")
		.option(
			"-o, --output-file <outputFile>",
			"Store compiled archive to a file",
		)
		.addOption(
			new Option(
				"-f, --format <type>",
				"Force archive format for directories",
