 ...
 */
export async function createArchive(inputDir, archiveFile, asFiles = false) {
    if (archiveFile.toLowerCase().endsWith(".wacz")) {
        return await createWACZ(inputDir, archiveFile, asFiles);
    }
    else if (archiveFile.toLowerCase().endsWith(".warc") ||
 ...
 * @throws Error if creation fails
 */
export async function createWACZ(inputDir, waczFile, asFiles = false) {
 ...
 */
export async function watchAndCreate(inputDir, archiveFile, asFiles = false) {
