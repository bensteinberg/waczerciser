...
import { WARCSerializer, LimitReader, AsyncIterReader, } from "warcio";
import fs from "fs";
 ...
 */
export async function extractArchive(inputFile, outputDir) {
    if (inputFile.toLowerCase().endsWith(".wacz")) {
        return await extractWACZ(inputFile, outputDir);
    }
    else if (inputFile.toLowerCase().endsWith(".warc") ||
 ...
 * @throws Error if extraction fails
 */
export async function extractWACZ(waczFile, outputDir) {
    // Validate inputs
 ...
 * @throws Error if extraction fails
 */
export async function extractWARC(warcFile, outputDir) {
    // Import required modules
