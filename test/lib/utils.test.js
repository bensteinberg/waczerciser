...
test('utils.hasFiles', async (t) => {
  ...
  await t.test('returns false for empty directory', withTempDir(async (tempDir) => {
...
test('utils.uriToFilePath', async (t) => {
  const cases = [
    { uri: 'https://example.com/path/page.html', expected: 'https:/com,example)/path/page.html' },
    { uri: 'https://example.com/path/', expected: 'https:/com,example)/path/__index__.html' },
...
test('utils.safeJoin', async (t) => {
  const cases = [
    { base: '/safe/path', unsafe: '../../../etc/passwd', expected: '/safe/path/etc/passwd' },
    { base: '/safe/path', unsafe: 'etc/../../../passwd/', expected: '/safe/path/passwd/' },
